...
func (ec *executionContext) __resolve_entities(ctx context.Context, representations []map[string]interface{}) []fedruntime.Entity {
	...
	resolveEntity := func(ctx context.Context, typeName string, rep map[string]interface{}, idx []int, i int) (err error) {
		...
		switch typeName {
		case "Course":
			resolverName, err := entityResolverNameForCourse(ctx, rep)
			if err != nil {
				return fmt.Errorf(`finding resolver for Entity "Course": %w`, err)
			}
			switch resolverName {
			...
			case "findCourseByID":
				...
				entity, err := ec.resolvers.Entity().FindCourseByID(ctx, id0)
				if err != nil {
					return fmt.Errorf(`resolving Entity "Course": %w`, err)
				}
		...
		case "Enrollment":
			resolverName, err := entityResolverNameForEnrollment(ctx, rep)
			if err != nil {
				return fmt.Errorf(`finding resolver for Entity "Enrollment": %w`, err)
			}
			switch resolverName {
			...
			case "findEnrollmentByID":
				...
				if err != nil {
					return fmt.Errorf(`unmarshalling param 0 for findEnrollmentByID(): %w`, err)
				}
				entity, err := ec.resolvers.Entity().FindEnrollmentByID(ctx, id0)
				if err != nil {
					return fmt.Errorf(`resolving Entity "Enrollment": %w`, err)
				}
		...
		case "User":
			...
			case "findUserByAuthUserID":
				id0, err := ec.unmarshalNID2string(ctx, rep["authUserId"])
				if err != nil {
					return fmt.Errorf(`unmarshalling param 0 for findUserByAuthUserID(): %w`, err)
				}
				entity, err := ec.resolvers.Entity().FindUserByAuthUserID(ctx, id0)
				if err != nil {
...
func entityResolverNameForCourse(ctx context.Context, rep map[string]interface{}) (string, error) {
	for {
		var (
		...
		return "findCourseByID", nil
	}
	return "", fmt.Errorf("%w for Course", ErrTypeNotFound)
}

func entityResolverNameForEnrollment(ctx context.Context, rep map[string]interface{}) (string, error) {
	for {
		var (
			m   map[string]interface{}
			val interface{}
		...
		return "findEnrollmentByID", nil
	}
	return "", fmt.Errorf("%w for Enrollment", ErrTypeNotFound)
}
...
func entityResolverNameForUser(ctx context.Context, rep map[string]interface{}) (string, error) {
	for {
		...
		_ = val
		m = rep
		if _, ok = m["authUserId"]; !ok {
			break
		}
		return "findUserByAuthUserID", nil
	}
